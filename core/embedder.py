...
def hybrid_search(query: str, top_k: int = 10, query_vec: np.ndarray = None):
    """
    ...
    rank_labels = ["⭐️ PERINGKAT 1", "PERINGKAT 2", "PERINGKAT 3", "PERINGKAT 4",
                   "PERINGKAT 5", "PERINGKAT 6", "PERINGKAT 7", "PERINGKAT 8",
                   "PERINGKAT 9", "PERINGKAT 10"]
    rank_idx = 0
    for idx in best_idx:
    ...
    top10 = [f"#{i+1} {questions[idx]}" for i, idx in enumerate(best_idx[:10])]  # top-10 FAQ
    ...
    return context, np.array([top_e5, top_bm25, top_rrf]), best_q, top10

...
from core.kbli_handler import (
    ...
    format_kbli_context, format_raw_results
)
...
async def chat(req: ChatRequest, _conc: None = Depends(_concurrent_chat_limit)):
    ...
    if is_kbli_query(req.pertanyaan):
        ...
        if not clean_query:
            jawaban = (
                "Data KBLI untuk pertanyaan Anda:\n\n"
                "1. KBLI — (tidak ada data spesifik)\n"
                "ℹ️ Coba kata kunci yang lebih spesifik atau cek langsung di:\n"
    ...
    context, scores, best_q, top10_all = await asyncio.to_thread(
        hybrid_search, _search_query, 10,
    ...
    if len(parts) > 1:
        ...
        for part in parts:
            ...
            if p_bm25 < 5.0:
                ...
                continue
            p_ctx, p_scores, _, p_top10 = await asyncio.to_thread(hybrid_search, part, 10)

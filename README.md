...
| 🔍 **KBLI Lookup** | Cari kode KBLI sesuai deskripsi usaha. Multi-query expansion + 3× concurrent API kbli.co.id + LLM re-rank & grouping |
...
│   ├── llm.py                ←   Multi-provider LLM, failover chain, build prompt
│   ├── query_logger.py       ←   Query evaluation logging (JSONL)
│   ├── kbli_handler.py       ←   KBLI: clean query, API kbli.co.id, pool & dedup
│   └── kbli_sectors.py       ←   KBLI: mapping kode → sektor (A–V)
│
├── security/                 ← 🔒 Lapisan pengaman
│   ├── rate_limiter.py       ←   Anti-spam (5/menit), trusted user
│   └── session.py            ←   Session: timeout 30 menit, watchdog
...
│   ├── kbli.md               ←   KBLI: system prompt re-rank & format output
│   ├── kbli_expand.md        ←   KBLI: system prompt query expansion
│   └── KBLI-FLOW-v2.md        ←   KBLI: dokumentasi alur v2
│
...
│   └── KBLI-IMPLEMENTATION.md  ← Dokumentasi lengkap KBLI
│
...
```
USER: "kbli jual baju online" atau "Kenapa mitra tidak bisa verifikasi nik?"
         │
         ▼
┌─ 1. INPUT SANITASI ──────────────────────────┐
...
│    ⚠️ Langsung return, gak lanjut ke step 5 │
└───────────────────────────────────────────────┘
         │ (Non-KBLI → lanjut)
         ▼
┌─ 5. GREETING? ──────────────────────────────┐
...
└───────────────────────────────────────────────┘
         │
         ▼
    📊 QUERY LOGGED (SQLite + query_log.jsonl)
```
...
Nara bisa mencari kode KBLI (Klasifikasi Baku Lapangan Usaha Indonesia) berdasarkan deskripsi usaha user. Fitur ini menggunakan **5 tahap** dengan **2 LLM call + 3 concurrent API call** ke [kbli.co.id](https://kbli.co.id/api-docs).
...
Trigger cukup mention kata **"kbli"** (case insensitive). Query dibersihkan dari kata-kata noise seperti: `kbli`, `kode`, `usaha`, `bisnis`, panggilan (`kak`, `bang`, `mas`), partikel (`sih`, `dong`, `deh`), dan KBLI codes 5-digit.

### Tahap 2: Query Expansion (LLM)

LLM (**DeepSeek V4 Flash**) generate **3 varian query** yang genuinely berbeda sudut pandang:
...
3 varian query dikirim **bersamaan** ke `https://kbli.co.id/api/search?q={query}` via `asyncio.gather()`. Masing-masing ambil top-5 hasil (level 5 — class-level KBLI).
...
3. **Format output** — dengan struktur rapi:

```
🔹 Sebagai [interpretasi]
**Kategori: X — Nama Kategori**
**KBLI XXXXX — Nama Kegiatan**
Deskripsi singkat (terjemahan dari data, bukan buatan sendiri)
→ **Cocok untuk:** [penjelasan spesifik]
```

Hasil selalu diakhiri dengan disclaimer:
...
- ❌ **Produksi vs Dagang vs Jasa** — LLM expansion cover 3 sudut pandang beda
- ❌ **Eceran vs Grosir** — LLM re-rank bedain dari deskripsi user asli
- ❌ **Multi-kegiatan usaha** — Grouping per interpretasi tampilkan beberapa opsi
- ❌ **API kadang ngaco** — Multi-query + concurrent call coverage + LLM re-rank fix
- ❌ **LLM hallucination** — Prompt explicitly larang nambah data dari pengetahuan sendiri
...
**Added**
- **KBLI v2: Multi-query expansion** — LLM generate 3 varian query dari deskripsi user
- **3× concurrent API call** ke kbli.co.id via `asyncio.gather()`
- **Pooling & dedup** by kode KBLI (top 2 per query, unique across queries)
- **LLM re-rank + grouping** per interpretasi — format baru: Kategori → KBLI → Deskripsi → "Cocok untuk"
- `prompts/kbli_expand.md` — system prompt untuk query expansion
- `docs/KBLI-IMPLEMENTATION.md` — dokumentasi lengkap KBLI

**Changed**
- `prompts/kbli.md` — format update: grouping per interpretasi + kategori di atas KBLI
- `core/kbli_handler.py` — tambah `parse_expand_response()`, `format_kbli_context_v2()`, `pool_and_dedup()`
- `core/llm.py` — tambah `load_kbli_expand_template()`, `build_kbli_expand_prompt()`
- `server.py` — KBLI flow v2: expand → API → pool → re-rank
- Daily limit: **25 → 50 chat/hari**

**Docs**
- Alur chat di README: tambah KBLI Check sebagai step 4 (sebelum Greeting)
- File structure: tambah kbli_handler.py, kbli_sectors.py, prompt KBLI
- Fitur table: tambah baris KBLI Lookup
- Section baru di README: **KBLI Lookup — Cara Kerja** (5 tahap algoritma + common mistakes)
...
**Added**
- **KBLI Lookup** — Nara bisa mencari kode KBLI sesuai deskripsi usaha
- `core/kbli_handler.py` — deteksi trigger (`\\bkbli\\b`), clean query dari noise words, API client ke kbli.co.id
- `core/kbli_sectors.py` — mapping kode KBLI 5-digit ke sektor kategori A–V
- `prompts/kbli.md` — system prompt untuk format output KBLI

**Changed**
- `server.py` — routing KBLI: trigger kata "kbli" → clean → API kbli.co.id → LLM format → output
- `core/llm.py` — tambah `load_kbli_template()`, `build_kbli_prompt()`
